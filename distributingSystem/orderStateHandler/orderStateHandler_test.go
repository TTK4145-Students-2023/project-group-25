...
func TestOrderStateHandler(t *testing.T) {
	...
	//Test case 3: handling of node reconnecting to NTW
	t.Run("Check change in matrix after reconnect", func(t *testing.T) {

		peerList := peers.PeerUpdate{
			New:   "ID1",
			Lost:  []string{},
			Peers: []string{"ID1", "ID2", "ID3"},
		}

		//Mocking Local RSM
		localData := make(dt.RequestStateMatrix)
		localData["ID1"] = dt.SingleNode_requestStates{{STATE_new, STATE_none},
			{STATE_none, STATE_none},
			{STATE_confirmed, STATE_none},
			{STATE_none, STATE_none}}

		localData["ID2"] = dt.SingleNode_requestStates{{STATE_none, STATE_none},
			{STATE_none, STATE_confirmed},
			{STATE_new, STATE_none},
			{STATE_none, STATE_confirmed}}

		localData["ID3"] = dt.SingleNode_requestStates{{STATE_confirmed, STATE_none},
			{STATE_none, STATE_none},
			{STATE_new, STATE_none},
			{STATE_confirmed, STATE_none}}
		...
		//ReconnectNodeOrders(peerList, localData, "ID1")

		MergeNewNodeOrders(peerList, localData)
