...
func MakeMsgInput() AllElevData_withID {
...
func MakeMsgOutput() AllElevData_withID {
	localIP, _ := localip.LocalIP()
	AllElevDat := map[string]ElevData{
		"one": ElevData{
			Behavior:    "idle",
...
func TestP2P() {
	allElevData_fromDist := make(chan AllElevData_withID, 1)
	ReqStateMatrix_fromOrderHandler := make(chan RequestStateMatrix, 1)
	allElevData_fromNTW := make(chan AllElevData_withID, 1)
	ReqStateMatrix_fromNTW := make(chan RequestStateMatrix, 1)

	allElevData_toDist := make(chan AllElevData_withID, 1)
	ReqStateMatrix_toOrderHandler := make(chan RequestStateMatrix, 1)
	allElevData_toNTW := make(chan AllElevData_withID, 1)
